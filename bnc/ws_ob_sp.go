...
func NewSpotObWs(ctx context.Context, logger *slog.Logger) *SpotObWs {
	if ctx == nil {
	...
	logger = logger.With("ws", "bnc_spot_ob")
	cfg := DefaultSpotPublicWsCfg()
...
func (w *SpotObWs) SubNewSymbols(symbols ...string) (unsubed []string, err error) {
	return w.sub(symbols...)
}

// func (w *WsSpObMsgHandler) UnsubSymbols(symbols ...string) (err error) {
// 	var params []string
// 	for _, symbol := range symbols {
// 		params = append(params, strings.ToLower(symbol)+"@depth")
// 	}
// 	err = w.ws.UnsubStream(params...)
// 	if err != nil {
// 		return err
// 	}
// 	for _, symbol := range symbols {
// 		w.hasOb.Delete(symbol)
// 		w.obCache.Delete(symbol)
// 		w.updateCache.Delete(symbol)
// 	}
// 	return nil
// }
...
// symbol is case insensitive
func (w *SpotObWs) NewCh(symbol string) <-chan ob.Data {
	return w.radio.Sub(symbol)
}

func (w *SpotObWs) RemoveCh(ch <-chan ob.Data) {
	w.radio.UnsubAll(ch)
}
...
func (w *SpotObWs) sub(symbols ...string) (unsubed []string, err error) {
	var params []string
	for _, symbol := range symbols {
		params = append(params, strings.ToLower(symbol)+"@depth")
	}
...
func (w *SpotObWs) handle(msg RawWsClientMsg) []ob.Data {
	data := msg.Data
	err := msg.Err
	if err != nil {
...
func (w *SpotObWs) makeAllEmptyObs(err error) []ob.Data {
...
func (w *SpotObWs) update(depthData WsDepthMsg) ob.Data {
...
func (w *SpotObWs) setError(symbol string, err error) ob.Data {
	empty := ob.NewData(cex.BINANCE, cex.SYMBOL_TYPE_SPOT, symbol)
	empty.SetErr(err)
	w.ods.SetKV(symbol, empty)
	return empty
...
func (w *SpotObWs) cacheRawData(depthData WsDepthMsg) error {
	symbol := depthData.Symbol
...
func (w *SpotObWs) needQueryOb(symbol string) bool {
...
func (w *SpotObWs) queryOb(symbol string) error {
	oldCache := w.cache.GetV(symbol)
	if len(oldCache) < 10 {
...
func (w *SpotObWs) updateOb(depthData WsDepthMsg) ob.Data {
	symbol := depthData.Symbol
